      ...
      <section className="mx-auto flex min-h-screen w-full max-w-6xl flex-col justify-between px-6 py-8 sm:px-10 lg:px-12">
        <header className="flex items-center justify-between border-b border-ink/15 pb-5">
          <div className="flex items-center gap-3">
            <div className="grid size-10 place-items-center border border-ink/20 bg-field text-ink">
              <DatabaseZap aria-hidden="true" className="size-5" />
            </div>
            <span className="text-sm font-semibold uppercase tracking-[0.18em] text-moss">
              Physics Question Bank
            </span>
          </div>
          <span className="text-sm text-ink/60">MVP foundation</span>
        </header>

        <div className="grid gap-12 py-14 lg:grid-cols-[1.2fr_0.8fr] lg:items-end">
          <div className="max-w-3xl">
            <p className="mb-5 text-sm font-semibold uppercase tracking-[0.18em] text-copper">
          ...
          <aside className="border-l-4 border-copper bg-field/70 p-6">
        ...
        <div className="grid gap-4 pb-4 md:grid-cols-3">
          {workflow.map((item) => (
            <article key={item.title} className="border border-ink/15 bg-paper p-5">
              <item.icon aria-hidden="true" className="mb-5 size-6 text-copper" />
              <h2 className="text-xl font-semibold">{item.title}</h2>
              <p className="mt-3 leading-7 text-ink/68">{item.detail}</p>
            </article>
          ))}
        </div>
      </section>
